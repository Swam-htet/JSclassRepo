...
//     console.log("Get review by Movie id : ", movieId);
//     try {
...
//     let reviewId = req.params["id"];
//     let review = req.body;
//     console.log(`Update review ID :${reviewId} , body => `, review);
//     try {
...
async function getReviewByMovieID(req, res, next) {
    ...
    let id = req.params["id"];
    console.log("Get review by movie ID: ", id);
    try {
        let review = await ReviewService.getReviewByMovieID(id);
        if (!review) {
            throw new Error("No such review.");
        } else {
            await res.status(200).json(review);
...
async function createReview(req, res, next) {
    ...
    try {
        ...
        if (!newReview) {
            ...
            await res.status(201).json(newReview);
        }
    } catch (e) {
        console.log("Error >> ", e);
        await res.status(400).json({message: e})
...
async function updateReviewByID(req, res, next) {
    let id = req.params["id"];
    console.log("Update review by ID: ", id);
    let body = req.body;
...
async function deleteReviewByID(req, res, next) {
    let id = req.params["id"];
    console.log("Delete review by ID: ", id);
    try {
